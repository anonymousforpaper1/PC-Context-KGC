...
os.makedirs("metrics_lb_temp", exist_ok=True)
csv_file_lb = f"metrics_lb_temp/metrics_{base_vars['dataset']}_{base_vars['input_confidence']}_{base_vars['threshold']}_lb_gw.csv"

if not os.path.exists(csv_file_lb):
    with open(csv_file_lb, "w", newline="") as f:
        writer = csv.writer(f)
...
if base_vars['type'] == 'all':
    ...
    for k in k_range:
        ...
        path_pred = f"Tests-AnyBURL/k_filtered_prediction_PC/{vars['dataset']}/lower_bound/merged_predictions_{vars['dataset']}_train_missing_{vars['em_iterations']}_{base_vars['input_confidence']}_k1to{k}_pc_{vars['threshold']}.txt"
        # Template
        lines = f"""\
        ...
        # Write config file
        config_path = f"config-eval-lb-{base_vars['dataset']}.generated.properties"
        ...
        if len(metrics) == 4:  # Ensure we have 4 values
            ...
            # Append to CSV
            with open(csv_file_lb, "a", newline="") as f:
                writer = csv.writer(f)
                writer.writerow([k, hits_1, hits_3, hits_10, mrr])
            print(f"Metrics for k={k} appended to {csv_file_lb}")
        else:
